...
2. 브라우저에서 다음 중 하나를 고릅니다.
   - **새 프로젝트 시작**: 의뢰서 작성 → 디자인 설정 → 작업 시작
   - **기존 프로젝트 가져오기**: 폴더 선택 → 읽기 전용 분석 → 승인 → 안전 복제
3. 허브의 최근 프로젝트에서 작업을 다시 엽니다.
...
동의한 사용자에게만 검색·탐색·프로젝트 사용 기록을 로컬에 저장합니다. 기록은 로컬 문서 검색과 추천 순서에만 사용하며 외부 전송하지 않습니다. 허브의 **내 메모리**에서 일시정지, 개별 삭제, 전체 삭제, 내보내기·가져오기를 할 수 있습니다. 가져온 프로젝트의 텍스트 색인은 가져오기 화면에서 별도로 선택합니다.
