...
def run_launcher(root, state, port, idle=30):
    ...
    while time.time() < deadline and runtime_path is None:
        for candidate in Path(state).glob("runtime-2.7.0-*.json"):
            try:
...
class WindowsRuntimeTests(unittest.TestCase):
    ...
    def test_02b_two_vas_copies_run_independently(self):
        ...
        try:
            ...
            self.assertGreaterEqual(len(list(self.state.glob("runtime-2.7.0-*.json"))), 2)
    ...
    def test_02c_stale_runtime_cannot_reuse_another_service(self):
        ...
        runtime_id = "2.7.0-" + hashlib.sha256(canonical).hexdigest()[:16]
        class ForgedHealth(http.server.BaseHTTPRequestHandler):
            def do_GET(self):
                body = json.dumps(
                    {"service": "VAS", "version": "2.7.0", "runtimeId": "wrong", "port": self.server.server_port}
                ).encode("utf-8")
        ...
        thread.start()
        stale = {
            "service": "VAS", "version": "2.7.0", "runtimeId": runtime_id,
    ...
    def test_05b_ai_handoff_preview_and_json_export(self):
        ...
        self.assertEqual(preview["document"]["generatedBy"]["version"], "2.7.0")
...
class WindowsRuntimeIdleTest(unittest.TestCase):
    def test_idle_server_stops_and_removes_runtime_file(self):
        with tempfile.TemporaryDirectory(prefix="vas-idle-test-") as temp:
            ...
            while time.time() < deadline and list(state.glob("runtime-2.7.0-*.json")):
                time.sleep(0.2)
            self.assertFalse(
                list(state.glob("runtime-2.7.0-*.json")),
