...
## 2026-09-08 VAS 2.7.4 흐름 재점검

- 삼중 따옴표·연속 문자열·중첩 인코딩과 과거 작업 기억의 출력 경계를 보완했습니다. 지식 필터 순서를 수정하고 이전 색인을 schema 3 재생성 전 제외합니다.
- 파생 추천 프로필 저장 실패, 삭제 후 늦은 확정, 결과 파일 읽기 경합·수동 중복 반영·이전 닫기 타이머, 잘못된 디자인 프로필을 보완했습니다.
- 공개 변경·검증 범위는 docs/releases/2.7.4.md, 운영 안내와 배포 확인은 옵시디언 VAS에 기록합니다.

## 2026-09-08 VAS 2.7.3 CI 후속 수정

- Windows CI에서 복사와 저장이 겹치면 확정 디자인을 두 번 쓰는 경합을 재현했습니다. 같은 확정 요청은 진행 중 쓰기를 공유하고 실패 후 재시도할 수 있게 수정했습니다.
- 시작 화면 테스트의 이동 완료 대기도 추가했습니다. 최종 배포는 2.7.3이며 2.7.2는 이력으로 남깁니다.

## 2026-09-08 VAS 2.7.2 통합 흐름·보안 보완

- 영구 저장 실패를 숨기는 대체 저장을 제거하고, 삭제 실패·재시도와 탭 간 디자인 초기화를 검증했습니다.
- 확정 디자인만 추천에 사용하며 파일명·긴 요구사항·인계 비밀값·호환 ZIP·RAG 색인·결과 검토 상태를 보완했습니다.
- 현재 동작과 문서·검색 인덱스를 맞추고, Windows 실제 서버 브라우저 검증을 CI에 추가했습니다.
- 검증: Python 210 통과·권한 1 건너뜀, 브라우저 88, 패키지 7, 압축 해제 Windows 실행 1, 배포 경계 4 통과.
- 사용자 운영 안내·워크플로우·감사 및 수정 근거는 옵시디언 VAS에 갱신합니다. 공개 상세 변경은 docs/releases/2.7.2.md.

## 2026-09-08 VAS 2.7.1 보안·글꼴 수정

- JSON 다운로드 직전 전체 인계를 정제하고 정제된 문서와 제공자 선택을 기준으로 해시를 계산합니다.
- 브라우저·Python·호환 소스 ZIP에 따옴표·중첩·접두사·유니코드·주석·템플릿 값을 포함한 공통 비밀값 검사를 적용했습니다.
- 명시적 작업 기억 삭제·전체 비우기·교체 가져오기는 이전·손상·임시 사본까지 정리하며 실패 시 성공으로 표시하지 않습니다.
- Windows 서버에 TTF MIME을 추가하고 실제 압축 해제본의 데스크톱·모바일에서 글꼴 로딩을 확인했습니다.
- 검증: Python 140 통과·심볼릭 링크 권한 1 건너뜀, 브라우저 77 통과, 패키지 7 통과, 압축 해제 Windows 실행 1 통과. 세부 기록은 docs/verification/privacy-font-fixes-2026-09-08.md.
- 운영·폴더 안내와 사용자용 업데이트 이력은 옵시디언 VAS에서 관리합니다. 기존 2.7.0 릴리스와 감사 원본은 이력으로 보존합니다.
