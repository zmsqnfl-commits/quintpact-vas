...
function Get-VASSafeRagMetadata {
    ...
    if ((Get-VASProjectValue $index 'schema' 0) -ne 3) { return @() }
...
function Export-VASProjectHandoff {
    ...
    try {
        $summary = [ordered]@{
            schema = 1; vasVersion = '2.7.5'; projectId = $project.projectId
...
"@
        ...
        return [ordered]@{ fileName = $safeName + '-VAS-2.7.5-handoff.zip'; bytes = [IO.File]::ReadAllBytes($zip) }
    } finally {
