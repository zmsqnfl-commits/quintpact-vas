...
function Get-VASLiveRuntime {
    ...
    try {
        ...
        if ($runtime.service -ne 'VAS' -or $runtime.version -ne '2.7.3' -or $runtime.runtimeId -ne $runtimeId -or -not $runtime.token -or [int]$runtime.port -lt 1) { throw 'invalid runtime' }
        ...
        if ($response.StatusCode -eq 200 -and $health.service -eq 'VAS' -and $health.version -eq '2.7.3' -and $health.runtimeId -eq $runtimeId -and [int]$health.port -eq [int]$runtime.port) {
            return $runtime
        }
    } catch { }
