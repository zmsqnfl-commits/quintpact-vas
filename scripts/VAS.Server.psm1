...
function Invoke-VASApiRequest {
    ...
    if ($path -eq '/api/status' -and $method -eq 'GET') {
        ...
        Write-VASResponse $Context 200 ([ordered]@{ service = 'VAS'; version = '2.7.6'; port = $State.Port; uptimeSeconds = $uptime; capabilities = $capabilities; memory = Get-VASMemoryStatus $State.MemoryRoot }); return
    }
    ...
    if ($path -eq '/api/projects/export' -and $method -eq 'POST') {
        ...
        try {
            ...
            $Context.Response.Headers['Content-Disposition'] = 'attachment; filename="VAS-2.7.6-handoff.zip"'
        ...
        while (-not $State.Shutdown -and $listener.IsListening) {
            ...
            try {
                ...
                if ($context.Request.Url.AbsolutePath -eq '/health') {
                    Write-VASResponse $context 200 ([ordered]@{ service = 'VAS'; version = '2.7.6'; runtimeId = $State.RuntimeId; port = $State.Port })
